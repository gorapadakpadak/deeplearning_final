...
# #     # 모델 저장
# #     model_save_path = os.path.join(opt.checkpoints_dir, f'sim_{opt.loss}_mobilenetV2_final.pth')
...
# #         plt.tight_layout()
# #         plt.savefig(f'{save_dir}/our_{key}_results.png')
# #         plt.close()
...
# # #     plot_results_smoothed(results, 'sim_mobilenetV2_loss_comparison_results_smoothedddd')
...
# # if __name__ == '__main__':
# #     opt.model = 'mobilenetV2'
# #     losses_to_compare = ['mse']  # 손실 함수 목록
# #     results = {}
# #     num_epochs_initial = 100  # 초기 학습 에포크
...
# #     # 결과 그래프 저장
# #     plot_results_smoothed(results, f'fianl_loss_comparison_{loss_function}_results_0000001_100')
...
# 모델 학습 및 평가 함수
def train_and_evaluate(opt, num_epochs):
    # print("안녕안녕안녕안녕안녕안녕",opt.csv_file,"안녕안녕안녕안녕안녕안녕")
    model = create_model(opt)
...
# # 옵션 파싱
# opt = TrainOptions().parse()
# print("Parsed options:", vars(opt))  # 디버깅 출력을 추가하여 opt 객체의 속성을 확인
...
# # 모델 학습 및 평가 함수
# def train_and_evaluate(opt, num_epochs):
#     print("CSV 파일 경로 확인:", opt.csv_file)  # 디버깅 출력을 추가하여 csv_file 속성을 확인
#     model = create_model(opt)
