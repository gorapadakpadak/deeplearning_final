...
def define_network(input_nc, lstm_hidden_size, model, init_from=None, isTest=False, gpu_ids=[]):
    ...
    if model == 'posenet':
        ...
        netG = GoogLeNetPose(input_nc, isTest=isTest, gpu_ids=gpu_ids)
    elif model == 'poselstm':
...
class RegressionHead(nn.Module):
    def __init__(self, lossID, weights=None, lstm_hidden_size=None):
        ...
        if lossID != "loss3":
            ...
            self.cls_fc_ori = weight_init_googlenet("Orientation", nn.Linear(nc_cls[0], 3))  # 수정된 부분
            ...
            self.cls_fc_ori = weight_init_googlenet("Orientation", nn.Linear(nc_cls[1], 3))  # 수정된 부분
    ...
    def forward(self, input):
        ...
        output_ori = self.cls_fc_ori(output)
        output_ori = F.normalize(output_ori, p=2, dim=1)
        return [output_xy, output_ori]
...
class PoseNet(nn.Module):
    def __init__(self, input_nc, weights=None, isTest=False,  gpu_ids=[]):
        ...
        self.cls3_fc = RegressionHead(lossID="loss3", weights=weights) # 최종 출력 6차원으로 변경
...
########### ResNet101 posenet ##############
#googlenet#
class GoogLeNetPose(nn.Module):
    def __init__(self, input_nc, isTest=False, gpu_ids=[]):
        super(GoogLeNetPose, self).__init__()
        self.gpu_ids = gpu_ids
        self.isTest = isTest
        googlenet = models.googlenet(pretrained=True)
        ...
        # Fully connected layers for position and orientation regression
